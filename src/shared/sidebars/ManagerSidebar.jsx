    ...
    <aside className="w-64 border-r border-[#e7edf3] dark:border-slate-800 bg-white dark:bg-background-dark flex flex-col h-screen sticky top-0">
      ...
      <div className="p-6">
        ...
        <nav className="flex flex-col gap-2">
          ...
          <SidebarItem icon="bar_chart" label="Workload Distribution" act="workload-distribution" navigate={navigate} />
          ...
          <SidebarItem icon="smart_toy" label="AI Assistant" act="ai-assistant" navigate={navigate} />
          <div className="relative">
            <button
              onClick={() => setSettingsDropdownOpen(!settingsDropdownOpen)}
              className="flex items-center justify-between w-full gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300"
            >
              <div className="flex items-center gap-3">
                <span className="material-symbols-outlined">settings</span>
                <p className="text-sm">Settings</p>
              </div>
              <span className="material-symbols-outlined text-sm transition-transform" style={{ transform: settingsDropdownOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                expand_more
              </span>
            </button>
            {settingsDropdownOpen && (
              <div className="ml-6 mt-1 space-y-1">
                <button
                  onClick={() => navigate('/manager/face-registration')}
                  className="flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 w-full"
                >
                  <span className="material-symbols-outlined text-sm">face</span>
                  <p className="text-sm">Face Registration</p>
                </button>
                <button
