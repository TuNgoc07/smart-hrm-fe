...
export default function SupportHubScreen() {
  const [messages, setMessages] = useState([
    { type: "ai", content: "Hello, I'm ready to help you with HR analytics and automated workflows. What would you like to investigate today?" }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { type: "ai", content: data.answer || "Sorry, I couldn't process that request." }]);
    } catch (error) {
        ...
        <footer className="sticky bottom-0 bg-white border-t p-4">
          <div className="space-y-3">
            ...
            <div className="flex gap-2 overflow-x-auto">
              <Suggestion text="Who forgot to check-in most?" onClick={() => setInputValue("Who forgot to check-in most?")} />
              <Suggestion text="Top 5 overtime earners" onClick={() => setInputValue("Top 5 overtime earners")} />
              <Suggestion text="Forecast payroll for next month" onClick={() => setInputValue("Forecast payroll for next month")} />
            </div>
            ...
            <div className="flex items-end gap-2 bg-slate-100 rounded-xl p-2">
              ...
              <button onClick={handleSendMessage} disabled={isLoading} className="bg-primary text-white p-2 rounded-lg disabled:opacity-50">
...
function Suggestion({ text, onClick }) {
  return (
    <button className="px-4 py-1.5 rounded-full border bg-white text-sm text-slate-600 hover:text-primary hover:border-primary" onClick={onClick}>
      {text}
    </button>
  );
}