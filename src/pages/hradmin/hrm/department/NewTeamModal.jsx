...
export default function NewTeamModal({ team, departments, onClose, onSaved }) {
  ...
  useEffect(() => {
    const fetchManagers = async () => {
      const res = await fetch(`${API_BASE_URL}/api/hradmin/employees?status=active`, {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.teamCode.trim()) { setError("Team code is required."); return; }
    if (!form.departmentId) { setError("Department is required."); return; }

    setLoading(true);
    try {
      const url = `${API_BASE_URL}/api/hradmin/teams`;
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          teamId: isEdit ? team.teamId : undefined,
