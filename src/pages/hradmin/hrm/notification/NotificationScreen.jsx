...
function PageHeading({ onMarkAllRead, unreadTotal }) {
    return (
...
function FilterBar({ activeFilter, onFilterChange, unreadCounts }) {
    return (
        <div className="flex gap-2 p-1 bg-slate-200/50 dark:bg-slate-800/50 rounded-xl w-fit">
            {FILTER_TABS.map(tab => {
                const typeKey = tab === 'All' ? 'total' : FILTER_TYPE_MAP[tab].toLowerCase();
                const count = unreadCounts[typeKey] || 0;
                return (
                    <button
                        key={tab}
...
export default function NotificationScreen() {
    ...
    const handleFilterChange = (filter) => {
        ...
        const typeFilter = filter === 'All' ? 'all' : FILTER_TYPE_MAP[filter];
        refresh(typeFilter);
    };
